...
int syshal_pressure_wake(void)
{
    ...
    syshal_timer_set_ms(MS5803_sampling_timer_priv, periodic, round((float) 1000.0f / sys_config.pressure_sample_rate.contents.sample_rate));
...
{
    return (syshal_timer_running(MS5803_sampling_timer_priv) == SYSHAL_TIMER_RUNNING) && initialised_priv;
}
