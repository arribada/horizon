...
TEST_F(Sm_MainTest, DISABLED_OperationalStateGPSAlwaysOnIoTTrigger)
{
    auto min_gps_updates = 100;
    ...
    gps_event.id = SYSHAL_GPS_EVENT_STATUS;
    gps_event.status.gpsFix = 2;
    syshal_gps_callback(&gps_event);

    gps_event.id = SYSHAL_GPS_EVENT_PVT;
...
TEST_F(Sm_MainTest, StatusRequestUSB)
{
    ...
    EXPECT_EQ(app_firmware_version, resp.cmd_status_resp.app_firmware_version);
    EXPECT_EQ(syshal_ble_version, resp.cmd_status_resp.ble_firmware_version);


    EXPECT_TRUE( 0 == std::memcmp( id_nordic, resp.cmd_status_resp.mcu_uid, sizeof( resp.cmd_status_resp.mcu_uid ) ) );
