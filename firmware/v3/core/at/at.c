...
int at_send_raw_fs(fs_handle_t handle, uint32_t length)
{
    ...
    DEBUG_PR_WARN("%s() function untested. Use with caution", __FUNCTION__);
...
int at_expect_http_header(uint32_t *length, uint16_t *http_code)
{
    ...
    /* Read HTTP code */
    uint32_t http_code_temp;
    status = at_expect(HTTP_expect, char_timeout_millisecond, &bytes_received, &http_code_temp);
