...
static void GPS_off_no_log(void)
{
    if (SM_GPS_STATE_ASLEEP == sm_gps_state)
        return; // GPS already asleep
...
void syshal_button_callback(syshal_button_event_t event)
{
    ...
    else if (event.released.duration_ms > BUTTON_CELLULAR_TEST_MS)
    {
    ...
    else if (event.released.duration_ms > BUTTON_GPS_TEST_MS)
    {
...
void syshal_gps_callback(const syshal_gps_event_t * event)
{
    ...
    // If gps data logging is disabled (maybe unnecessary?)
    ...
    switch (event->id)
    {
        ...
        case SYSHAL_GPS_EVENT_STATUS:
            ...
            else if (SM_GPS_STATE_ASLEEP != sm_gps_state)
            {
...
static void status_req(cmd_t * req, size_t size)
{
    ...
    resp->cmd_status_resp.app_firmware_version = version;
    ...
    syshal_device_id(&(resp->cmd_status_resp.mcu_uid));
...
static void sm_main_operational(sm_handle_t * state_handle)
{
    ...
    if ( (test_state_satellite == SM_TEST_WAITING) && !is_test_active_or_finishing_or_led_active() )
    {
        ...
        syshal_led_set_blinking(SYSHAL_LED_COLOUR_BLUE, LED_BLINK_TEST_PASSED_DURATION_MS);
        led_finish_time = 0;

        /* Already init in the operational state */
        if (!sm_iot_trigger_force(IOT_RADIO_SATELLITE)) // Pass test
        {
            syshal_led_set_solid(SYSHAL_LED_COLOUR_BLUE);
        }
        else // Fail test
        {
            syshal_led_set_blinking(SYSHAL_LED_COLOUR_BLUE, LED_BLINK_FAIL_DURATION_MS);
        }
